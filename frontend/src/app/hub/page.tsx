...
function ToolCard({
  ...
  const card = (
    ...
    >
      <div
        className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl ${soon ? "bg-slate-200 text-slate-400" : "text-white"}`}
      ...
      <div className="flex-1">
        <div className="mb-1 flex items-center gap-2.5">
          <h3 className={`text-[17.5px] font-extrabold tracking-tight ${soon ? "text-slate-500" : ""}`}>{name}</h3>
          <span
            className={`rounded px-2 py-0.5 text-[10.5px] font-extrabold uppercase tracking-wide ${
        ...
        <p className={`max-w-[520px] text-[13.5px] leading-snug ${soon ? "text-slate-400" : "text-slate-500"}`}>
          {description}
        </p>
      </div>
    ...
    <div className="flex min-h-screen flex-col bg-slate-50">
      ...
      <div className="flex flex-1 flex-col items-center px-8 pt-14">
        <div className="mb-10 max-w-[720px] text-center">
          ...
          <h1 className="mb-2.5 text-[34px] font-extrabold tracking-tight">Welcome back{user ? `, ${user.name}` : ""}</h1>
          <p className="text-[15.5px] leading-relaxed text-slate-500">
        ...
        <div className="flex w-full max-w-[720px] flex-col gap-4 pb-16">
          <ToolCard
