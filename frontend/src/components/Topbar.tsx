...
export function Topbar({ toolName }: { toolName?: string }) {
  ...
  return (
    <div className="flex h-16 items-center gap-6 border-b border-slate-200 bg-white px-7">
      <Link href="/hub" className="flex items-center gap-2.5 text-[16px] font-extrabold tracking-tight">
        <div
      ...
      {toolName ? (
        <div className="flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-100 py-1.5 pl-2.5 pr-3 text-[13px] font-semibold text-slate-700">
        ...
        {menuOpen && (
          <div className="absolute right-0 top-12 w-56 rounded-xl border border-slate-200 bg-white p-1.5 shadow-lg">
            ...
            <button
              type="button"
              onClick={logout}
              className="mt-1 w-full rounded-lg px-3 py-2 text-left text-[13px] font-semibold text-rose-600 hover:bg-rose-50"
            >
              Log out
            </button>
          </div>
