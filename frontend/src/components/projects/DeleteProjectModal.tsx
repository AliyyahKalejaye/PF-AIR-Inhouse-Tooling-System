...
// Delete confirmation modal — screen 13/17 from the approved mockups.
// Reused from both the Projects list page (row 3-dot menu) and the
// Project detail page (header 3-dot menu), so it owns the actual
// deleteProject call itself rather than pushing that back onto two
// separate callers.
...
export function DeleteProjectModal({ token, project, onCancel, onDeleted }: Props) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : "Couldn't delete that project. Please try again.");
      setDeleting(false);
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/55 px-4">
      <div className="w-full max-w-[440px] rounded-2xl bg-white p-7 text-center shadow-[0_30px_60px_-12px_rgba(15,23,42,.35)]">
        ...
        <h2 className="mb-2 text-[18px] font-extrabold tracking-tight">Delete this project?</h2>
        <p className="mb-5 text-[13.5px] leading-relaxed text-slate-500">
          This will permanently remove <b className="font-bold text-slate-700">&quot;{project.title}&quot;</b> and all
          its attached files, MIL, and history. This can&apos;t be undone.
        </p>
